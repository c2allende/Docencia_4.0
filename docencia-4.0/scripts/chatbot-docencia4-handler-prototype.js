...
function addMessage(text, sender = 'bot', type = 'bot', save = true) {
    ...
    msgDiv.className = `chat-message ${sender} ${type === 'warning' ? 'warning' : ''}`;
...
function showCategories() {
    ...
    FAQ_KNOWLEDGE_BASE.forEach(item => {
        ...
        chip.onclick = () => {
            handleUserSubmit(item.question);
        };
...
function renderWelcomeMessage() {
    ...
    disclaimerDiv.className = 'chat-message bot warning';
    ...
    addMessage('¡Hola! Soy el Asistente Docencia 4.0. ¿En qué le puedo ayudar hoy?', 'bot', 'bot', false);
    showCategories();
}
...
function loadConversation() {
    ...
    if (history) {
        ...
        const chips = dom.messagesContainer.querySelectorAll('.suggestion-chip');
        chips.forEach(chip => {
